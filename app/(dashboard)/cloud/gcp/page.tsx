...
import { gcpCspmPostures, gcpCspmSummary } from "@/lib/mock-data"
...
export default function CloudGcpPage() {
  const [activeTab, setActiveTab] = useState("overview")
  const [enabledPostures, setEnabledPostures] = useState<number[]>(gcpCspmPostures.map((posture) => posture.id))
  ...
  const criticalFindings = gcpCspmPostures.filter(
    (posture) => posture.status === "critical" && enabledPostures.includes(posture.id),
  )

  const pieChartData = [
    { name: "Compliant", value: gcpCspmSummary.compliantResources },
    { name: "Warning", value: gcpCspmSummary.warningResources },
    { name: "Critical", value: gcpCspmSummary.criticalResources },
  ]

  return (
    <div className="flex flex-col h-full">
      <PageHeader
        title="GCP Security Posture"
        description="Cloud Security Posture Management for Google Cloud Platform"
      />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{gcpCspmSummary.complianceScore}%</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-xs text-muted-foreground">Last scan: {gcpCspmSummary.lastScan}</div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{gcpCspmSummary.totalResources}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-xs text-muted-foreground">
                {gcpCspmSummary.compliantResources} compliant resources
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{gcpCspmSummary.warningResources}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{gcpCspmSummary.criticalResources}</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card className="bg-[#0f1d24] border-orange-600/20">
                ...
                <CardContent>
                  <div className="h-[300px]">
                    <BarChart
                      data={gcpCspmSummary.categoryCompliance}
                      index="category"
          ...
          <TabsContent value="posture-checks" className="space-y-4">
            <Card className="bg-[#0f1d24] border-orange-600/20">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-orange-500">GCP Security Posture Checks</CardTitle>
