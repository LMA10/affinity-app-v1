...
import { azureCspmPostures, azureCspmSummary } from "@/lib/mock-data"
...
export default function CloudAzurePage() {
  const [activeTab, setActiveTab] = useState("overview")
  const [enabledPostures, setEnabledPostures] = useState<number[]>(azureCspmPostures.map((posture) => posture.id))
  ...
  const criticalFindings = azureCspmPostures.filter(
    (posture) => posture.status === "critical" && enabledPostures.includes(posture.id),
  )

  const pieChartData = [
    { name: "Compliant", value: azureCspmSummary.compliantResources },
    { name: "Warning", value: azureCspmSummary.warningResources },
    { name: "Critical", value: azureCspmSummary.criticalResources },
  ]

  return (
    <div className="flex flex-col h-full">
      <PageHeader title="Azure Security Posture" description="Cloud Security Posture Management for Azure" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{azureCspmSummary.complianceScore}%</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-xs text-muted-foreground">Last scan: {azureCspmSummary.lastScan}</div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{azureCspmSummary.totalResources}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-xs text-muted-foreground">
                {azureCspmSummary.compliantResources} compliant resources
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{azureCspmSummary.warningResources}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-[#0f1d24] border-orange-600/20">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-orange-500">{azureCspmSummary.criticalResources}</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card className="bg-[#0f1d24] border-orange-600/20">
                ...
                <CardContent>
                  <div className="h-[300px]">
                    <BarChart
                      data={azureCspmSummary.categoryCompliance}
                      index="category"
          ...
          <TabsContent value="posture-checks" className="space-y-4">
            <Card className="bg-[#0f1d24] border-orange-600/20">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-orange-500">Azure Security Posture Checks</CardTitle>
            ...
            <Card className="bg-[#0f1d24] border-orange-600/20">
              ...
              <CardContent>
                <div className="h-[400px]">
                  <AreaChart
                    data={azureCspmSummary.trendData}
                    index="date"
                    categories={["compliant", "warning", "critical"]}
